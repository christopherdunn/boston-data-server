...
async function fetchAllCrimeRecords(q, filters) {
    const params = {
    ...
    for (let i = 0; i < offsets.length; i += PARALLEL_REQUESTS) {
        const chunkOffsets = offsets.slice(i, i + PARALLEL_REQUESTS);
        const chunkResults = await Promise.all(chunkOffsets.map(offset => {
            const chunkParams = {
                resource_id: CRIME_RESOURCE_ID,
                limit: PAGE_SIZE.toString(),
                offset: offset.toString()
            ...
            if (q)
                chunkParams.q = q;
            if (filters && Object.keys(filters).length)
                chunkParams.filters = JSON.stringify(filters);
...
function countAndSummarize(records, field, top) {
    const counts = {};
...
server.tool("get_crime_reports", {
...
}, async ({ street, offense_description, district, year, month, day_of_week, hour, shooting, }) => {
    // Build q string from uppercased fuzzy fields
    let qParts = [];
    if (street)
        qParts.push(street.trim().toUpperCase());
    if (offense_description)
        qParts.push(offense_description.trim().toUpperCase());
    const q = qParts.length ? qParts.join(" ") : undefined;
    ...
    if (street) {
        ...
        records = records.filter((r) => (r.STREET || "").toUpperCase().includes(streetNorm));
    }
    if (offense_description) {
        const descNorm = offense_description.trim().toUpperCase();
        records = records.filter((r) => (r.OFFENSE_DESCRIPTION || "").toUpperCase().includes(descNorm));
    }
    if (day_of_week) {
        const dayNorm = day_of_week.trim().toUpperCase();
        records = records.filter((r) => (r.DAY_OF_WEEK || "").trim().toUpperCase() === dayNorm);
    }
...
async function fetchCheckbookChunk(q, filters, offset) {
    const params = {
...
async function fetchAllCheckbookRecords(q, filters) {
    const params = {
        resource_id: CHECKBOOK_RESOURCE_ID,
        limit: '1',
        offset: '0'
    ...
    for (let i = 0; i < offsets.length; i += PARALLEL_REQUESTS) {
        const chunkOffsets = offsets.slice(i, i + PARALLEL_REQUESTS);
        const chunkResults = await Promise.all(chunkOffsets.map(offset => fetchCheckbookChunk(q, filters, offset)));
        chunkResults.forEach(records => allRecords.push(...records));
    }
    return allRecords;
}
// MCP tool
server.tool("get_checkbook_records", {
    ...
    let filters = {};
    if (fiscal_year)
        filters["Fiscal Year"] = fiscal_year;
    if (year)
        filters["Year"] = year;
    if (month)
        filters["Month"] = month;
